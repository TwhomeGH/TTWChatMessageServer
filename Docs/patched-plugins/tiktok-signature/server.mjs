...
async function initBrowser() {
  ...
  try {
    ...
    const cacheSignedUrl = (url, referer = "") => {
      try {
        const parsed = new URL(url);
        const path = parsed.pathname;
        signedUrlCache.set(path, { url, capturedAt: Date.now(), referer });
      } catch (e) {}
    };
    ...
    page.on("request", (request) => {
      try {
        const u = request.url();
        // Also cache WebSocket URLs (ws/wss) — they contain valid X-Bogus
        if (u.startsWith("ws") && u.includes("X-Bogus")) {
          cacheSignedUrl(u, request.headers().referer || "");
          return;
        }
...
async function initWithLocalSdk() {
  ...
  // Inject v5.1.3 SDK (byted_acrawler) before any page scripts run
  await page.evaluateOnNewDocument((sdkCode) => {
    try {
      eval(sdkCode);
      console.log("[SDK] v5.1.3 injected via evaluateOnNewDocument");
    } catch (e) {
      console.error("[SDK] v5.1.3 injection error:", e.message);
    }
  }, localSdkContent);

  // Also inject v2.0.0 SDK (__sdkN) for the _signDirectly fast path
  if (sdk485) {
    await page.evaluateOnNewDocument((sdkCode485) => {
      try {
        eval(sdkCode485);
        console.log("[SDK] v2.0.0 injected via evaluateOnNewDocument");
      } catch (e) {
        console.error("[SDK] v2.0.0 injection error:", e.message);
      }
    }, sdk485);
  }
  ...
  const sdkStatus = await page.evaluate(() => {
    ...
    const hasSdkN = !!(window.__sdkN && window.__sdkN.u && window.__sdkN.u[995]);
    const keys = window.byted_acrawler
      ? Object.keys(window.byted_acrawler).slice(0, 10)
      : [];
    return { hasAcrawler, hasFrontierSign, hasSdkN, keys };
  });

  console.log("[Server] SDK status:", JSON.stringify(sdkStatus));

  if (!sdkStatus.hasSdkN && !sdkStatus.hasFrontierSign) {
    throw new Error(
...
/**
 * Sign a WebSocket URL by navigating the browser to the live page of the
 * target room, letting TikTok's actual page SDK generate a valid X-Bogus.
 * Returns the signed URL with a valid X-Bogus for webcast-ws.tiktok.com.
 */
async function _signForWebSocket(targetUrl, navigateToUrl = null) {
  await initBrowser();
  ...
  // Navigate to the room's live page so TikTok's page SDK emits a real WS URL
  const livePage = navigateToUrl || `https://www.tiktok.com/@${urlObj.searchParams.get("room_id") || "zara"}/live`;
  console.log(`[WSS] Navigating to ${livePage} to capture WebSocket URL...`);
  ...
  try {
    ...
    console.log(`[WSS] Navigation failed (${e.message}), falling back immediately`);
  }

  if (!navigated) {
    return _signDirectly(cleanUrl);
  }

  // Wait for a fresh WebSocket URL with X-Bogus to be captured
  const WAIT_MS = 5000;
  while (Date.now() - callStart < WAIT_MS) {
    // Check all cached URLs captured after our navigation started
    for (const [path, cached] of signedUrlCache) {
      if (cached.capturedAt >= callStart) {
        const u = cached.url;
        if (!u.startsWith("ws")) continue;
        cookies = await page.cookies();
        generationCount++;
        const result = parseResult(u);
        const xb = new URL(u).searchParams.get("X-Bogus");
        console.log(`[WSS] Captured X-Bogus from ${path}: ${xb}`);
        return result;
      }
    }
  ...
  // Fallback: try the local SDK and hope
  console.warn(`[WSS] No fresh WS URL captured within ${WAIT_MS / 1000}s, falling back to local SDK`);
  return _signDirectly(cleanUrl);
}
...
async function _generateSignedUrlInternal(
  ...
  const attempt = async () => {
    if (fetchUrl.startsWith("wss://")) {
      console.log(`[Server] WSS URL detected, using direct signer`);
      // Try _signForWebSocket first (captures from page navigation),
      // fall back to _signDirectly (local SDK computation) on failure.
      try {
        return await _signForWebSocket(fetchUrl, navigateTo);
      } catch (e) {
        console.log(`[Server] _signForWebSocket failed (${e.message}), falling back to _signDirectly`);
...
async function _signDirectly(fetchUrl, userAgent = null) {
  const out = await page.evaluate((url) => {
    ...
    try {
      ...
      msToken = msTokenMatches.length
        ? msTokenMatches[msTokenMatches.length - 1][1]
        : "";
    } catch (e) {
      // document.cookie access denied (about:blank / restricted origin)
    }
