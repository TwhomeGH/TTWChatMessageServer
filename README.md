...
Twitch 訂閱者（或主播本人）可在聊天室輸入 `G#Ad` 指令投放自訂廣告至疊加層。
...
| `<訊息>` | 廣告文字（必填） |
| `tts` | 選用，啟用文字轉語音 |
| `icon=<網址>` | 選用，自訂圖示（預設使用 Twitch 頭貼） |
| `user=<名稱>` | 選用，自訂顯示名稱 |
| `interval=<分鐘>` | 選用，自動重複間隔（最低 15 分鐘） |

### 間隔設定

- `interval=30` → 每 30 分鐘自動重複發送一次
- `interval=0` 或不設 → 單次發送，不重複
- 低於 15 分鐘 → 自動調整為 15 分鐘，並發送 Bark + 疊加層通知

### 審核模式

贊助廣告管理頁面 `http://localhost:3332/sponsor` 可切換三種審核模式：

| 模式 | 行為 |
|------|------|
| **不需審核**（預設） | 送出即顯示 + 啟動定時器 |
| **過濾器自動審核** | 跑 `processFilter()`，沒被擋→自動通過；被擋→拒絕 + Bark 通知 |
| **手動審核** | 標記為「待審核」，需管理員手動通過/拒絕 |
...
資料儲存於 `sponsor_ads.json`，定時器在程式重啟後不會自動恢復。
